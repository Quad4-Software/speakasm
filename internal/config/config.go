...
func Load(args []string) (Config, error) {
	cfg := Default()
	fs := flag.NewFlagSet("speakasm", flag.ContinueOnError)
	...
	if v, ok := os.LookupEnv("SPEAKASM_ADDR"); ok && v != "" {
		cfg.Addr = v
	}
	if v, ok := os.LookupEnv("SPEAKASM_WEB"); ok && v != "" {
		cfg.WebRoot = v
	}
	if v, ok := os.LookupEnv("SPEAKASM_READ_TIMEOUT"); ok && v != "" {
		d, err := time.ParseDuration(v)
		if err != nil {
			return Config{}, fmt.Errorf("SPEAKASM_READ_TIMEOUT: %w", err)
		}
		cfg.ReadTimeout = d
	}
	if v, ok := os.LookupEnv("SPEAKASM_WRITE_TIMEOUT"); ok && v != "" {
		d, err := time.ParseDuration(v)
		if err != nil {
			return Config{}, fmt.Errorf("SPEAKASM_WRITE_TIMEOUT: %w", err)
		}
		cfg.WriteTimeout = d
	}
