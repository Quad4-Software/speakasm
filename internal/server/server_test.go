...
import (
	...
	"github.com/Quad4-Software/speakasm/internal/server"
)
...
func TestAPIAndStatic(t *testing.T) {
	...
	res2 := mustGet(ctx, t, client, ts.URL+"/api/voices")
	defer closeBody(t, res2)
	var payload map[string]any
	if err := json.NewDecoder(res2.Body).Decode(&payload); err != nil {
		t.Fatal(err)
	}
	voices, ok := payload["voices"].([]any)
	if !ok || len(voices) == 0 {
		t.Fatalf("voices payload: %#v", payload)
	}
	...
	res4 := mustGet(ctx, t, client, ts.URL+"/api/voices/does-not-exist")
...
func TestPWAAssets(t *testing.T) {
	...
	files := map[string]string{
		...
		"manifest.webmanifest": `{"name":"speakasm","start_url":"/"}`,
	}
